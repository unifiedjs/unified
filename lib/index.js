...
export class Processor extends CallableInstance {
    ...
    function executor(resolve, reject) {
      ...
      self.run(parseTree, realFile, function (error, tree, file) {
        if (error || !tree || !file) {
          ...
          const result = self.stringify(compileTree, file)
      ...
      function realDone(error, file) {
        if (error || !file) {
          ...
          // @ts-expect-error: `done` is defined if `resolve` is not.
          done(undefined, file)
        }
      }
    }
  ...
  processSync(file) {
    ...
    // The result will be set by `this.process` on this file.
    const realFile = /** @type {VFileWithOutput<CompileResult>} */ (vfile(file))

    this.process(realFile, realDone)
  ...
  run(tree, file, done) {
     ...
     */
    function executor(resolve, reject) {
      // @ts-expect-error: `file` can’t be a `done` anymore, we checked.
      const realFile = vfile(file)
   ...
   *
   * @overload
   * @param {[plugin: Plugin<Parameters, Input, Output>, enable: boolean] | [plugin: Plugin<Parameters, Input, Output>, ...parameters: Parameters]} tuple
   * @returns {UsePlugin<ParseTree, HeadTree, TailTree, CompileTree, CompileResult, Input, Output>}
   *
   * @param {[plugin: Plugin, ...parameters: Array<unknown>] | PluggableList | Plugin | Preset | null | undefined} value
   *   Usable value.
  ...
  use(value, ...options) {
    ...
    if (value === null || value === undefined) {
      ...
      if (Array.isArray(value)) {
        // @ts-expect-error: look at tuples?
        addList(value)
      } else {
        addPreset(value)
      }
